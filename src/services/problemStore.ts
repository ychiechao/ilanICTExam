...
const FIRST_HIDDEN_CASE_NUMBER = 3;
...
export async function loadProblems(): Promise<Problem[]> {
  ...
  return readJson<Problem[]>(LOCAL_PROBLEMS_KEY, sampleProblems).map(applyHiddenCasePolicy);
}
...
export async function importProblemsFromJson(
  ...
  const problems = source.items.map((item) => applyHiddenCasePolicy(normalizeProblem(item, source.year)));
...
export async function importProblemsFromCsv(
  ...
  const problems = rows.slice(1).map((row) => applyHiddenCasePolicy(normalizeProblem(csvRowToProblem(headers, row))));
  return importProblems(problems, mode);
}
...
export async function saveProblem(problem: Problem) {
  problem = applyHiddenCasePolicy(problem);
  if (db) {
...
function normalizeCases(value: unknown, groupTitle = "測資"): ProblemCase[] {
  ...
  return value.map((item, index) => {
    ...
    return {
      ...
      visibility: shouldHideCase(caseTitle, index, record.visibility) ? "hidden" : "public",
    };
  });
}

function applyHiddenCasePolicy(problem: Problem): Problem {
  const sourceCases = Array.isArray(problem.cases) ? problem.cases : [];
  const sourceExamples = Array.isArray(problem.examples) ? problem.examples : [];
  const cases: ProblemCase[] =
    sourceCases.length > 0
      ? sourceCases.map((testCase, index): ProblemCase => ({
          ...testCase,
          visibility: shouldHideCase(testCase.caseTitle, index, testCase.visibility) ? "hidden" : "public",
        }))
      : sourceExamples.map((example, index): ProblemCase => ({
          groupTitle: shouldHideCase(`C${index + 1}`, index, "public") ? "隱藏測資" : "公開測資",
          caseTitle: `C${index + 1}`,
          input: example.input,
          output: example.output,
          score: 10,
          visibility: shouldHideCase(`C${index + 1}`, index, "public") ? "hidden" : "public",
        }));
  const hasHiddenCase = cases.some(
    (testCase, index) => (readCaseNumber(testCase.caseTitle) ?? index + 1) >= FIRST_HIDDEN_CASE_NUMBER,
  );
  const hiddenExample = sourceExamples[FIRST_HIDDEN_CASE_NUMBER - 1];

  if (!hasHiddenCase && hiddenExample?.input && hiddenExample.output) {
    cases.push({
      groupTitle: "隱藏測資",
      caseTitle: `C${FIRST_HIDDEN_CASE_NUMBER}`,
      input: hiddenExample.input,
      output: hiddenExample.output,
      score: 10,
      visibility: "hidden",
    });
  }

  const publicCaseCount = cases.filter((testCase) => testCase.visibility === "public").length;
  const examples = publicCaseCount > 0 ? sourceExamples.slice(0, publicCaseCount) : sourceExamples;
  return { ...problem, examples, cases };
}

function shouldHideCase(caseTitle: string, index: number, visibility: unknown) {
  if (visibility === "hidden") {
    return true;
  }

  const caseNumber = readCaseNumber(caseTitle) ?? index + 1;
  return caseNumber >= FIRST_HIDDEN_CASE_NUMBER;
}

function readCaseNumber(caseTitle: string) {
  const match = caseTitle.match(/\d+/);
